...
describe("Socket.io: sincronização em tempo real", () => {
  ...
  it("card:label-added chega em tempo real quando uma etiqueta é aplicada", async () => {
    const tokenA = await registerAndGetToken("a@teste.com");
    const boardRes = await request(app)
      .post("/boards")
      .set("Authorization", `Bearer ${tokenA}`)
    ...
    const eventPromise = waitForEvent(clientB, "card:label-added");
    ...
    const payload = await eventPromise;
    expect(payload).toEqual({ cardId, labelId });
  });

  it("checklist-item:created chega em tempo real quando um item é adicionado", async () => {
    const tokenA = await registerAndGetToken("a@teste.com");
    const boardRes = await request(app)
      .post("/boards")
      .set("Authorization", `Bearer ${tokenA}`)
    ...
    const eventPromise = waitForEvent(clientB, "checklist-item:created");

    await request(app)
      .post(`/cards/${cardId}/checklist-items`)
      .set("Authorization", `Bearer ${tokenA}`)
      .send({ text: "Item ao vivo" });

    const payload = await eventPromise;
    expect(payload.item.text).toBe("Item ao vivo");
    expect(payload.cardId).toBe(cardId);
  });
